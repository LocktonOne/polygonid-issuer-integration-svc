...
func GetCredentialQRCode(w http.ResponseWriter, r *http.Request) {
	...
	code, response, err := issuer_sender.GetCredentialQRCode(r, request.Did, request.Claim)
	if err != nil {
		helpers.Log(r).WithError(err).Error("failed to get credential QR-code")
	...
	w.WriteHeader(code)
	ape.Render(w, converter.ToQrCodeResource(*response))
}
